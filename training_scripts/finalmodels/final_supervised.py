...
def train_supervised(args, labeled_wsi, val_wsi, logs_dir):
    ...
    if not args.hpc:
        ...
        train_dl_lab_medium = create_balanced_dataloader(dataset_labeled_medium, num_workers= args.workers, batch_size=args.bs, hpc = True)
        ...
        test_dataset_medium = LMDBTorchDataset(lmdb_path="D:/tfm_data/preprocessed/dataset_lvl3_fmt", allowed_wsi_ids=val_wsi, ds_type='test')
        ...
        train_dl_lab_high = create_balanced_dataloader(dataset_labeled_high, num_workers= args.workers, batch_size=int(args.bs/2), hpc = True)
        dataset_labeled_medium = LMDBTorchDataset("./dataset_lvl4",ds_type='train', allowed_wsi_ids=labeled_wsi, included_sampling_categories= [0,1])
        train_dl_lab_medium = create_balanced_dataloader(dataset_labeled_medium, num_workers= args.workers, batch_size=int(args.bs/2), hpc = True, total_images=len(dataset_labeled_high))
    ...
    for epoch in range(args.max_epochs):
        ...
        for (lab_high, target_high), (lab_medium, target_medium) in tqdm(train_labeled_loader,desc=f'Training Epoch {epoch+1}'):
            optimizer.zero_grad()
            ...
            if iter_num % 1000 == 0:
                ...
                dices_per_res = [np.mean(dice_score) for dice_score in dice_score_per_resolution.values()]
                ...
                if current_dhgp_score > best_dhgp:
                    torch.save(model.state_dict(), model_dhgp) # Also save the model with the best dHGP dice score
