...
from src.utils.dataset_unlabeled import LMDBTorchDatasetUnlabeled, HardnessBatchSampler, FastHardnessBatchSampler, fit_normal, HardnessNormalBatchSampler
...
parser.add_argument("--max_iterations", type = int, default = 40001)
parser.add_argument("--warmup_iterations", type = int, default = 2000) # 5% of total training time.
...
parser.add_argument("--lr", type = float, default=8e-4) # From swin unetr paper it would be 0.0008
...
def train_UPC(args, labeled_wsi, val_wsi, unlabeled_wsi, logs_dir):
    print("Training", labeled_wsi)
    print("Validation", val_wsi)
    ...
    last_model_path = os.path.join(logs_dir,"last_model.pth")
    model_dhgp = os.path.join(logs_dir,"best_dhgp.pth")
    student = swin_upc()
    meanteacher = MeanTeacher(student_model=student)
    meanteacher.student.cuda()
    meanteacher.teacher.cuda()

    optimizer = torch.optim.AdamW(params = meanteacher.student.parameters(),lr = args.lr ,weight_decay=5e-4)
    warmup_scheduler = torch.optim.lr_scheduler.LinearLR(optimizer=optimizer, start_factor=1/10, end_factor=1, total_iters=args.warmup_iterations) # 5% of total training time
    cosine_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=args.max_iterations-args.warmup_iterations, eta_min=1e-6)
    lr_scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer=optimizer, schedulers=[warmup_scheduler, cosine_scheduler],milestones=[args.warmup_iterations])
    ...
    # Custom Patch Filtering Setup
    hardness_predictions_file = np.load("./src/config/hardness_unlabeled.npz",allow_pickle=True)
    hardness_prediction_values = hardness_predictions_file["hardness"]
    hardness_keys = hardness_predictions_file["idx"] # IMPORTANT: These are the composite keys consisting of WSIID_COORDX_COORDY
    pred_hardness_per_key = {str(key):value for key,value in zip(hardness_keys,hardness_prediction_values)}
    # mean,std = fit_normal(hardness_prediction_values,clip_values=[0.05,0.8]) # Filter out both extremes
    # mean = 0 # Start with Mean Zero and go to the right from there
    mean,std = 0,0.18 # FINAL. With 0.36 target mean this means that at the end of training 
    threshhold_hardness = 0.0 # Starting Value
    ...
    if not args.hpc:
        ...
        dataset_unlabeled = LMDBTorchDatasetUnlabeled("D:/tfm_data/preprocessed/lvl4_unlabeled",allowed_wsi_ids=unlabeled_wsi,return_wsi_id=True)
        unlabeled_sampler = HardnessNormalBatchSampler(dataset_unlabeled, batch_size=3, threshold_hardness=threshhold_hardness,mean_hardness=mean,std_hardness=std, hardness_per_key=pred_hardness_per_key)
        train_dl_unlab = torch.utils.data.DataLoader(dataset_unlabeled, num_workers= 0,pin_memory=False,persistent_workers=False,batch_sampler=unlabeled_sampler)
        ...
        dataset_unlabeled = LMDBTorchDatasetUnlabeled("./lvl4_unlabeled_normalized",allowed_wsi_ids=unlabeled_wsi,return_wsi_id=False, return_idx=True)
        unlabeled_sampler = HardnessNormalBatchSampler(dataset_unlabeled, batch_size=16, threshold_hardness=threshhold_hardness,mean_hardness=mean, std_hardness=std,hardness_per_key=pred_hardness_per_key)
        train_dl_unlab = torch.utils.data.DataLoader(dataset_unlabeled, num_workers= args.workers,pin_memory=True,persistent_workers=True,batch_sampler=unlabeled_sampler)
        ...
        test_loader_high = torch.utils.data.DataLoader(dataset=test_dataset_high, batch_size=16, num_workers=args.workers, pin_memory=True, persistent_workers=True)
        test_loader_medium = torch.utils.data.DataLoader(dataset=test_dataset_medium, batch_size=16, num_workers=args.workers, pin_memory=True, persistent_workers=True)
    ...
    length_unlabeled = len(dataset_unlabeled)
    patches_to_log_per_loader = 0  # Log 4 random images per loader (total up to 12)
    val_losses_per_resolution = {}
    loaders = [
        (test_loader_high, "high-res"),
        (test_loader_medium, "medium-res")
        # (test_loader_low, "low-res")
    ]
    test_info = {}
    ...
    rampup_iterations = 5000 # CHANGE  instead of dynamic (which worked with 10k iterations, fixes 5k since this is a much longer run)   int(max_iterations * RAMPUP_PERCENTAGE)
    # args.consistency_rampup = rampup_iterarampup_iterations
    ...
    for epoch in range(args.max_epochs):
        ...
        for (((lab_high, target_high), (lab_medium, target_medium)),(unlab_images, unlab_wsi_ids)) in tqdm(zip(train_labeled_loader,train_dl_unlab),desc=f'Training Epoch {epoch+1}', total=min(len(train_dl_lab_high),len(train_dl_unlab))):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                # Calculate Entropy Threshhold Using Uncertainty
                average_UC = mask_UC.mean() # Between Zero and 1
                # unlabeled_sampler.update_threshhold(average_UC=average_UC) # Update the adaptive sampler
                unlabeled_sampler.update_normal_params_UC(average_UC= average_UC)
            ...
            # Plot Entropy Threshhold Over Time
            writer.add_scalar("params/hardness_threshhold",unlabeled_sampler.threshold_hardness, iter_num)
            ...
            writer.add_scalar("params/mean_normal",unlabeled_sampler.mean_hardness, iter_num)
            writer.add_scalar("params/std_normal",unlabeled_sampler.std_hardness, iter_num)
            ...
            if iter_num % 1000 == 0:
                    ...
                    for loader, res_tag in loaders:
                        ...
                        for val_idx, batch in enumerate(loader):
                            ...
                            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                                ...
                                l_ce = criterion_ce(out, targets).mean()
                                total_loss_eval = l_ce + dice_loss_eval
                        ...
                        dhgp_dice.append(per_class_dice[1])
                ...
                if global_dice > best_performance:
                    ...
                    torch.save(meanteacher.teacher.state_dict(), last_model_path)
                    patience_counter +=1
                
                current_dhgp_score = np.mean([x.detach().cpu().item() for x in dhgp_dice])
                if current_dhgp_score > best_dhgp:
                    torch.save(meanteacher.teacher.state_dict(), model_dhgp) # Also save the model with the best dHGP dice score
                    best_dhgp = current_dhgp_score
                    logger.info(f"Saved new best_dhgp model with value {best_dhgp}")
...
if __name__ == "__main__":
    ...
    logger.info(f"Length of Unlabeled WSI IDs before filtering: {len(unlabeled_wsi_ids)}")

    with open("./src/config/final_unlabeled_validation_config.json") as f:
        unlabeled_validation_config = json.load(f)
    ...
    logger.info(f"Length of Unlabeled WSI IDs after filtering: {len(unlabeled_wsi_ids)}")
