...
def train_UPC(args, labeled_wsi, val_wsi, unlabeled_wsi, logs_dir):
    ...
    for epoch in range(args.max_epochs):
        ...
        for (((lab_high, target_high), (lab_medium, target_medium), (lab_low, target_low)),(unlab_images)) in tqdm(zip(train_labeled_loader,train_dl_unlab),desc=f'Training Epoch {epoch+1}', total=min(len(train_dl_lab_high),len(train_dl_unlab))):
...
if __name__ == "__main__":
    ...
    experiment_name = f"{timestamp}_split{args.split}_{args.max_iterations}epochs_{args.bs}bs"
