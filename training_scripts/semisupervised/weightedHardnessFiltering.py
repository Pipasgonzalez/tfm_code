...
from src.utils.dataset_unlabeled import LMDBTorchDatasetUnlabeled, HardnessBatchSampler, get_unlabeled_weighted_hardnesssampler
...
def train_UPC(args, labeled_wsi, val_wsi, unlabeled_wsi, logs_dir):
    ...
    alpha_hardness_filtering = 0.999 
    ...
    if not args.hpc:
        ...
        unlabeled_sampler = get_unlabeled_weighted_hardnesssampler(dataset_unlabeled=dataset_unlabeled, pred_hardness_per_key=pred_hardness_per_key)
        train_dl_unlab = torch.utils.data.DataLoader(dataset_unlabeled, batch_size=16, num_workers= 0,pin_memory=False,persistent_workers=False,sampler=unlabeled_sampler)
        ...
        unlabeled_sampler = get_unlabeled_weighted_hardnesssampler(dataset_unlabeled=dataset_unlabeled, pred_hardness_per_key=pred_hardness_per_key)
        train_dl_unlab = torch.utils.data.DataLoader(dataset_unlabeled,batch_size=16, num_workers= args.workers,pin_memory=True,persistent_workers=True,sampler=unlabeled_sampler)
...
if __name__ == "__main__":
    ...
    unlabeled_wsi_ids = unlabeled_wsi_config["all_unlabeled_wsi_ids"]
    ...
    unlabeled_wsi_ids = list(set(unlabeled_wsi_ids) - set(labeled_wsi) - set(test_wsi) - set(unlabeled_wsi_ids))
