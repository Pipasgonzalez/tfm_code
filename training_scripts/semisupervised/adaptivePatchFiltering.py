...
from src.utils.dataset_unlabeled import LMDBTorchDatasetUnlabeled, HardnessBatchSampler, FastHardnessBatchSampler
...
def train_UPC(args, labeled_wsi, val_wsi, unlabeled_wsi, logs_dir):
    ...
    # Custom Patch Filtering
    # entropy_predictions_file = np.load("./src/data/entropy_predictions.npz",allow_pickle=True)
    ...
    alpha_hardness_filtering = 0.9999 

    if not args.hpc:
        dataset_labeled_high = LMDBTorchDataset("D:/tfm_data/preprocessed/lvl3_black_removed_normalized",ds_type='train', allowed_wsi_ids=labeled_wsi)
        dataset_labeled_medium = LMDBTorchDataset("D:/tfm_data/preprocessed/lvl4_black_removed_normalized",ds_type='train', allowed_wsi_ids=labeled_wsi)
        dataset_labeled_low = LMDBTorchDataset("D:/tfm_data/preprocessed/lvl5_black_removed_normalized",ds_type='train', allowed_wsi_ids=labeled_wsi)
        ...
        dataset_unlabeled = LMDBTorchDatasetUnlabeled("D:/tfm_data/preprocessed/lvl4_unlabeled_black_removed_normalized",allowed_wsi_ids=unlabeled_wsi,return_wsi_id=True) # CHANGE returning whole slide images now
        unlabeled_sampler = HardnessBatchSampler(dataset_unlabeled, batch_size=3, threshold_hardness=threshhold_hardness,hardness_per_key=pred_hardness_per_key)
        ...
        test_dataset_high = LMDBTorchDataset(lmdb_path="D:/tfm_data/preprocessed/lvl3_black_removed_normalized", allowed_wsi_ids=val_wsi, ds_type='test')
        ...
        test_dataset_medium = LMDBTorchDataset(lmdb_path="D:/tfm_data/preprocessed/lvl4_black_removed_normalized", allowed_wsi_ids=val_wsi, ds_type='test')
        ...
        test_dataset_low = LMDBTorchDataset(lmdb_path="D:/tfm_data/preprocessed/lvl5_black_removed_normalized", allowed_wsi_ids=val_wsi, ds_type='test')
        ...
        train_dl_lab_medium = create_balanced_dataloader(dataset_labeled_medium, num_workers= args.workers, batch_size=6, total_images=len(dataset_labeled_high),hpc = True)
        ...
        # unlabeled_sampler = HardnessBatchSampler(dataset_unlabeled, batch_size=16, threshold_hardness=threshhold_hardness,hardness_per_key=pred_hardness_per_key)
        unlabeled_sampler = FastHardnessBatchSampler(dataset_unlabeled, batch_size=16, threshold_hardness=threshhold_hardness,hardness_per_key=pred_hardness_per_key)
    ...
    for epoch in range(args.max_epochs):
        ...
        for (((lab_high, target_high), (lab_medium, target_medium), (lab_low, target_low)),(unlab_images, unlab_wsi_ids)) in tqdm(zip(train_labeled_loader,train_dl_unlab),desc=f'Training Epoch {epoch+1}', total=min(len(train_dl_lab_high),len(train_dl_unlab))):
            ...
            #     kornia_all=dataset_labeled_high.kornia_weak,
            #     min_ops=0, max_ops=3
            # )
            ...
            del labeled_images, labeled_targets, out_supervised_student,out_supervised_teacher, lab_high, lab_medium, lab_low, target_high, target_medium, target_low
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                unlabeled_sampler.threshold_hardness = (alpha_hardness_filtering * unlabeled_sampler.threshold_hardness) + ((1-alpha_hardness_filtering) * average_UC.item()) # EMA ENTROPY THRESHHOLD THAT GROES AS MODEL GETS SMARTER
...
if __name__ == "__main__":
    ...
    for grp, ids in unlabeled_wsi_config.items():
