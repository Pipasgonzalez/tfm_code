...
def train_UPC(args, labeled_wsi, val_wsi, unlabeled_wsi, logs_dir):
    ...
    if not args.hpc:
        ...
        # dataset_labeled_low = LMDBTorchDataset("D:/tfm_data/preprocessed/lvl5_labeled_normalized",ds_type='train', allowed_wsi_ids=labeled_wsi, included_sampling_categories=[0,1])
        ...
        # train_dl_lab_low = create_balanced_dataloader(dataset_labeled_low, num_workers= 0, batch_size=1, total_images=100,hpc =False)
        ...
        # test_dataset_low = LMDBTorchDataset(lmdb_path="D:/tfm_data/preprocessed/lvl5_labeled_normalized", allowed_wsi_ids=val_wsi, ds_type='test')
        # test_loader_low = torch.utils.data.DataLoader(dataset=test_dataset_low, batch_size=1, num_workers=0, pin_memory=False, persistent_workers=False)
    else:
        ...
        # train_dl_lab_low = create_balanced_dataloader(dataset_labeled_low, num_workers= args.workers, batch_size=4, total_images=len(dataset_labeled_high),hpc = True)
        ...
        # test_loader_low = torch.utils.data.DataLoader(dataset=test_dataset_low, batch_size=32, num_workers=args.workers, pin_memory=True, persistent_workers=True)
    ...
    for epoch in range(args.max_epochs):
        ...
        for (((lab_high, target_high), (lab_medium, target_medium)),(unlab_images, unlab_wsi_ids)) in tqdm(zip(train_labeled_loader,train_dl_unlab),desc=f'Training Epoch {epoch+1}', total=min(len(train_dl_lab_high),len(train_dl_unlab))):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                logits = out_unsupervised_student.float() # for numerical stability
                probs = torch.softmax(logits, dim=1)
                log_probs = torch.log(probs + EPS)
                p_log_p = probs * log_probs
                pixel_entropy = -torch.sum(p_log_p, dim=1)
                avg_patch_entropy = pixel_entropy.mean(dim=(1, 2))
                training_hardness_values.extend(avg_patch_entropy.float().cpu().tolist())
            ...
            if iter_num % 100 == 0: # 100 Normal Distribution Updates for 10 000 iterations
                # unlabeled_sampler.update_normal_params(training_hardness_values=training_hardness_values)
                # unlabeled_sampler.update_normal_params_UC(average_UC= average_UC)
                training_hardness_values.clear()
            # writer.add_scalar("params/excluded_samples", float(np.sum(unlabeled_sampler.sampling_weights == 0.0)/length_unlabeled), iter_num)
