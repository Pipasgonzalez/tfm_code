...
def pre_train(args, labeled_wsi_ids, test_wsi_ids, logs_dir,model_save_dir,skip_validation=False):
    ...
    wsi_to_indices = {}
    for idx, (key, wsi_id) in enumerate(train_dataset.metadata):
        if wsi_id not in wsi_to_indices:
            wsi_to_indices[wsi_id] = []
        wsi_to_indices[wsi_id].append(idx)
    unique_wsi_ids = list(wsi_to_indices.keys())
    # unique_wsi_ids = np.unique(train_dataset.get_wsi_ids()).tolist()
    ...
    model = get_model(nclasses=num_classes, device="cuda")
    ...
    for i in range(max_epoch):
        ...
        random.shuffle(unique_wsi_ids)
        pool_a_wsi_ids = random.sample(unique_wsi_ids, int(len(unique_wsi_ids) / 2))
        pool_b_wsi_ids = list(set(unique_wsi_ids) - set(pool_a_wsi_ids))

        pool_a_indices = [idx for wsi_id in pool_a_wsi_ids for idx in wsi_to_indices[wsi_id]]
        pool_b_indices = [idx for wsi_id in pool_b_wsi_ids for idx in wsi_to_indices[wsi_id]]

        pool_a_dataset = Subset(train_dataset, pool_a_indices)
        pool_b_dataset = Subset(train_dataset, pool_b_indices)

        train_dl_lab = create_balanced_dataloader(
            pool_a_dataset,
            batch_size=bs,
        ...
        train_dl_unlab = create_balanced_dataloader(
            pool_b_dataset,
            batch_size=bs,
            replacement=True, #False,
            num_workers=workers
        ...
        # pseudo_labeled_wsi = random.sample(unique_wsi_ids, int(len(unique_wsi_ids)/2))
        # pseudo_labeled_ds = LMDBTorchDataset(
        #     lmdb_path=lmdb_path, allowed_wsi_ids=pseudo_labeled_wsi
        # )
        # remaining_wsi_ids = list(set(unique_wsi_ids) - set(pseudo_labeled_wsi))
        # pseudo_unlabeled_ds = LMDBTorchDataset(
        #     lmdb_path=lmdb_path, allowed_wsi_ids=remaining_wsi_ids
        # )
        ...
        #     replacement=True #False,
        # )
        logger.info(f"Starting Training. Length of Labeled: {len(train_dl_lab)} Length of Pseudo-Unlabeled: {len(train_dl_unlab)}")
        progress_bar_train = tqdm(zip(train_dl_lab, train_dl_unlab),desc=f"Epoch {i+1}/{max_epoch}", ascii=True, total=min(len(train_dl_lab), len(train_dl_unlab)))
        for batch_lab, batch_unlab in progress_bar_train:
            optimizer.zero_grad()
            patch_lab, target_lab = batch_lab
            patch_unlab, target_unlab = batch_unlab

            # Now move each tensor to CUDA
            patch_lab, target_lab = patch_lab.cuda(), target_lab.cuda()
            patch_unlab, target_unlab = patch_unlab.cuda(), target_unlab.cuda()

            img_mask, loss_mask = generate_mask(patch_lab)

            # NN Input: Fused Image with Copy Paste

            net_input = patch_lab * img_mask + patch_unlab * (1 - img_mask)
            with autocast(device_type="cuda"):
                out_mixl = model(net_input)
                loss_dice, loss_ce = bcp_loss(
                    output=out_mixl,img_l=target_lab, patch_l=target_unlab, mask=loss_mask, ce_loss=ce_loss, dc_loss=dc_loss, u_weight=1.0, unlab=True
                )
        ...
        if not skip_validation:
            ...
            writer.add_scalar("DiceScoreEval/Mean", mean_dice, i + 1)
...
if __name__ == "__main__":
    ...
    experiment_name = f"exp_{args.pretrain_epochs}preepochs_{args.selftrain_epochs}_selfepochs_{timestamp}" # f"{args.exp}_pretrepochs{args.pretrain_epochs}_{timestamp}"
