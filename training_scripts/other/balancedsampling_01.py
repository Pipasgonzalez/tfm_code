...
from src.utils.dataset_unlabeled import LMDBTorchDatasetUnlabeled, EntropyBatchSampler
...
def train_UPC(args, labeled_wsi, val_wsi, unlabeled_wsi, logs_dir):
    ...
    if not args.hpc:
        ...
        dataset_unlabeled = LMDBTorchDatasetUnlabeled("D:/tfm_data/preprocessed/lvl4_unlabeled_black_removed_normalized",allowed_wsi_ids=unlabeled_wsi,return_wsi_id=True, included_sampling_categories = [0,1]) # CHANGE ALL SAMPLING CATEGORIES FOR THIS EXPERIMENT
        # CHANGE This is where the random sampling occurs
        ...
        dataset_labeled_high = LMDBTorchDataset("./dataset_lvl4",ds_type='train', allowed_wsi_ids=labeled_wsi)
        dataset_labeled_medium = LMDBTorchDataset("./dataset_lvl5",ds_type='train', allowed_wsi_ids=labeled_wsi)
        dataset_labeled_low = LMDBTorchDataset("./dataset_lvl6",ds_type='train', allowed_wsi_ids=labeled_wsi)
        
        train_dl_lab_high = create_balanced_dataloader(dataset_labeled_high, num_workers= args.workers, batch_size=10, hpc = True)
        train_dl_lab_medium = create_balanced_dataloader(dataset_labeled_medium, num_workers= args.workers, batch_size=4, total_images=len(dataset_labeled_high),hpc = True)
        train_dl_lab_low = create_balanced_dataloader(dataset_labeled_low, num_workers= args.workers, batch_size=2, total_images=len(dataset_labeled_high),hpc = True)
        ...
        train_dl_unlab = torch.utils.data.DataLoader(dataset_unlabeled,batch_size=16, num_workers= args.workers if args.hpc else 0,pin_memory=True,drop_last=True,persistent_workers=True,sampler=unlabeled_sampler)

        test_dataset_high = LMDBTorchDataset(lmdb_path="./dataset_lvl4", allowed_wsi_ids=val_wsi, ds_type='test')
        test_dataset_medium = LMDBTorchDataset(lmdb_path="./dataset_lvl5", allowed_wsi_ids=val_wsi, ds_type='test')
        test_dataset_low = LMDBTorchDataset(lmdb_path="./dataset_lvl6", allowed_wsi_ids=val_wsi, ds_type='test')
    ...
    for epoch in range(args.max_epochs):
        ...
        for (((lab_high, target_high), (lab_medium, target_medium), (lab_low, target_low)),(unlab_images, unlab_wsi_ids)) in tqdm(zip(train_labeled_loader,train_dl_unlab),desc=f'Training Epoch {epoch+1}', total=min(len(train_dl_lab_high),len(train_dl_unlab))):
            ...
            writer.add_scalar("params/entropy_threshhold",unlabeled_sampler.threshold_entropy, iter_num)
