...
def apply_unlabeled_augs_batch(images, kornia_weak, kornia_strong, min_weak_ops=0, max_weak_ops=2, min_strong_ops=1, max_strong_ops=1):
    # images: (B, C, H, W) GPU tensor, [0,1] float
...
def train_UPC(args, labeled_wsi, val_wsi, unlabeled_wsi, logs_dir):
    ...
    cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=args.max_iterations, eta_min=5e-5)
    lr_scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer=optimizer, schedulers=[warmup,cosine], milestones=[500])
