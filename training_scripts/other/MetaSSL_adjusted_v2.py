...
def train_UPC(args, labeled_wsi, val_wsi, unlabeled_wsi, logs_dir):
    ...
    # lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=optimizer,T_0=1000, T_mult=2,eta_min=1e-6)
    # warmup = torch.optim.lr_scheduler.LinearLR(optimizer=optimizer, start_factor=0.1, total_iters=500)
    # cos_anneal = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=#args.max_iterations, eta_min=1e-4)
    # lr_scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer=optimizer, schedulers=[warmup,cos_anneal], milestones=[1000])
    ...
    if not args.hpc:
        ...
        train_dl_lab_4 = create_balanced_dataloader(dataset_labeled_4, num_workers= args.workers, batch_size=10, hpc = True)
        train_dl_lab_5 = create_balanced_dataloader(dataset_labeled_5, num_workers= args.workers, batch_size=4, total_images=len(dataset_labeled_4),hpc = True)
        train_dl_lab_6 = create_balanced_dataloader(dataset_labeled_6, num_workers= args.workers, batch_size=2, total_images=len(dataset_labeled_4),hpc = True)
        ...
        test_loader_4 = torch.utils.data.DataLoader(dataset=test_dataset_4, batch_size=32, num_workers=args.workers, pin_memory=True, persistent_workers=True)
        test_loader_5 = torch.utils.data.DataLoader(dataset=test_dataset_5, batch_size=32, num_workers=args.workers, pin_memory=True, persistent_workers=True)
        test_loader_6 = torch.utils.data.DataLoader(dataset=test_dataset_6, batch_size=32, num_workers=args.workers, pin_memory=True, persistent_workers=True)
    ...
    for epoch in range(args.max_epochs):
        ...
        for (((lab_4, target_4), (lab_5, target_5), (lab_6, target_6)),(unlab_images, unlab_wsi_ids)) in tqdm(zip(train_labeled_loader,train_dl_unlab),desc=f'Training Epoch {epoch+1}', total=min(len(train_dl_lab_4),len(train_dl_unlab))):
            ...
            labeled_images, labeled_targets = apply_labeled_augs_batch(
                ...
                kornia_all=dataset_labeled_4.kornia_weak,
                min_ops=0, max_ops=3
            )
