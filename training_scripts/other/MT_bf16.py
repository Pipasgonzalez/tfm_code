...
from src.networks.swin_unet import get_swinunet, get_swinunet_max_beef, get_swinunet_fast
...
def pre_train(args, labeled_wsi_ids, test_wsi_ids, logs_dir,model_save_dir,skip_validation=False):
    ...
    train_dl = create_balanced_dataloader(dataset=train_dataset, batch_size=bs, replacement=True, num_workers=workers, total_images=100)
    ...
    focal_weight = 4.0 # Focal Loss ca 1/4 of the dice loss
    dice_weight = 1.0
    ...
    for i in range(max_epoch):
        ...
        if not skip_validation:
            ...
            patches_to_log = 10  # How many patches to log
            ...
            with torch.no_grad():
                ...
                for val_idx,batch in enumerate(progress_bar_val):
                    ...
                    with autocast(device_type="cuda",dtype=torch.bfloat16):
                        out = model(images)
                        dice_loss_eval = dice_loss_fn(out, target.unsqueeze(1)) * dice_weight
                        focal_loss_eval = focal_loss_fn(out,target.unsqueeze(1)) * focal_weight
                    ...
                    # --- START: New Best/Worst Batch Logic ---
                    # We use the batch-averaged 'total_loss_eval' as the score
                    loss_val = total_loss_eval.item()

            
                    
                    # For Logging always the same images to see progress
                    if images_logged_this_epoch < patches_to_log and (val_idx - last_log_batch_idx) > 4:
                        ...
                        for b in range(images_np.shape[0]):
                            ...
                            if total_pixels > 0 and (class_1_pixels / total_pixels) >= 0.1:
                                ...
                                tag_prefix = f"Validation/Patch_{wsi_ids[b]}"
            ...
            # # Log the 4 best batches
            # print(f"\n[Epoch {i+1}] Logging {len(best_batches)} best validation batches...")
            # for idx, batch_data in enumerate(best_batches):
            #     log_batch_to_tensorboard(batch_data, prefix=f"Best_{idx+1}", epoch=i+1, writer=writer)

            # # Log the 4 worst batches
            # print(f"[Epoch {i+1}] Logging {len(worst_batches)} worst validation batches...")
            # for idx, batch_data in enumerate(worst_batches):
            #     log_batch_to_tensorboard(batch_data, prefix=f"Worst_{idx+1}", epoch=i+1, writer=writer)
...
if __name__ == "__main__":
    ...
    # python -m src.training_scripts.MT_bf16 --model unet --multiplier_unet 2 --pretrain_bs 8 --lmdb_path D:/tfm_data/preprocessed/dataset_lvl3 --workers 0 --optimizer adamw --pretrain_epochs 10 --split 2
    if args.hpc:
