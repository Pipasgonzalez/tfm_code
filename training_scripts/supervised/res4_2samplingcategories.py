...
def train_supervised(args, labeled_wsi, val_wsi, logs_dir):
    ...
    if not args.hpc:
        dataset_labeled_high = LMDBTorchDataset("/content/lvl3_local",ds_type='train', allowed_wsi_ids=labeled_wsi, included_sampling_categories=[0,1]) # 2 are the boring patches
        ...
        test_loader_high = torch.utils.data.DataLoader(dataset=test_dataset_high, batch_size=args.bs*2, num_workers=2, pin_memory=True, persistent_workers=True)
    else:
        dataset_labeled_high = LMDBTorchDataset("./dataset_lvl4",ds_type='train', allowed_wsi_ids=labeled_wsi, included_sampling_categories= [0,1])
        train_dl_lab_high = create_balanced_dataloader(dataset_labeled_high, num_workers= args.workers, batch_size=args.bs, hpc = True)
        ...
        for (patches, targets) in tqdm(train_dl_lab_high,desc=f'Training Epoch {epoch+1}'):
            ...
            if iter_num % 500 == 0:
                ...
                if global_dice > best_performance:
                    ...
                    torch.save(model.state_dict(), model_path_last) # So we can also have the last model
                    patience_counter +=1
...
if __name__ == "__main__":
    ...
    # CHANGE Add Test WSI to Labeled WSI
    logger.info(f"Length of Labeled WSI before merging")
    labeled_wsi = labeled_wsi + test_wsi
    logger.info(f"Length of Labeled WSI after merging")
