...
from src.utils.dataset_supervised import LMDBTorchDataset, create_balanced_dataloader, FaberSampler
...
def train_supervised(args, labeled_wsi, val_wsi, logs_dir):
    ...
    if not args.hpc:
        dataset_labeled_high = LMDBTorchDataset("/content/lvl3_local",ds_type='train', allowed_wsi_ids=labeled_wsi, included_sampling_categories=[0,1,2]) # 2 are the boring patches
        faber_sampler = FaberSampler(dataset_labeled_high, samples_per_class=4)
        train_dl_lab_high = torch.utils.data.DataLoader(dataset = dataset_labeled_high, num_workers=args.workers, pin_memory=True, persistent_workers=True, batch_sampler=faber_sampler)
        
        test_dataset_high = LMDBTorchDataset(lmdb_path="/content/lvl3_local", allowed_wsi_ids=val_wsi, ds_type='test')
        test_loader_high = torch.utils.data.DataLoader(dataset=test_dataset_high, batch_size=4, num_workers=args.workers, pin_memory=True, persistent_workers=True)
    else:
        dataset_labeled_high = LMDBTorchDataset("./dataset_lvl3",ds_type='train', allowed_wsi_ids=labeled_wsi, included_sampling_categories= [0,1,2])
        faber_sampler = FaberSampler(dataset_labeled_high, samples_per_class=4)  # totals to 16 batch size, 4 for 4 classes = 16
        train_dl_lab_high = torch.utils.data.DataLoader(dataset=dataset_labeled_high, num_workers=args.workers, persistent_workers=True, pin_memory=True, batch_sampler=faber_sampler)
    ...
    logger.info(f"Length of the Training Dataset: {len(dataset_labeled_high)}")
    logger.info(f"Length of the Test Dataset: {len(test_dataset_high)}")

    patches_to_log_per_loader = 4  # Log 4 random images per loader (total up to 12)
    val_losses_per_resolution = {}
    loaders = [
        (test_loader_high, "res4")
    ]
    test_info = {}
    ...
    for epoch in range(args.max_epochs):
        ...
        for (patches, targets) in tqdm(train_dl_lab_high,desc=f'Training Epoch {epoch+1}'):
            optimizer.zero_grad()
            ...
            patches, targets = patches.cuda(non_blocking=True), targets.cuda(non_blocking=True)

            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                # Supervised forward/loss
                out_supervised = model(patches)
                dice_loss_supervised = dice_loss(out_supervised, targets.unsqueeze(1))
                focal_loss_supervised = focal_loss(out_supervised, targets.unsqueeze(1))
