...
def train_supervised(args, labeled_wsi, val_wsi, logs_dir):
    ...
    if not args.hpc:
        dataset_labeled_high = LMDBTorchDataset("D:/tfm_data/preprocessed/dataset_lvl3_fmt",ds_type='train', allowed_wsi_ids=labeled_wsi, included_sampling_categories=[0,1,2]) # 2 are the boring patches
        train_dl_lab_high = torch.utils.data.DataLoader(dataset = dataset_labeled_high, batch_size=4, num_workers=0, pin_memory=False, persistent_workers=False)
        ...
        train_dl_lab_high = torch.utils.data.DataLoader(dataset=dataset_labeled_high, batch_size = args.bs, num_workers=args.workers, persistent_workers=True, pin_memory=True)
