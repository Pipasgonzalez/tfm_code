...
def train_supervised(args, labeled_wsi, val_wsi, logs_dir):
    ...
    if not args.hpc:
        dataset_labeled_high = LMDBTorchDataset(lmdb_path="/content/lvl3_local",ds_type='train', allowed_wsi_ids=labeled_wsi, included_sampling_categories=[0,1,2]) # 2 are the boring patches
        ...
        test_loader_high = torch.utils.data.DataLoader(dataset=test_dataset_high, batch_size=args.bs, num_workers=2, pin_memory=True, persistent_workers=True)
    else:
            ...
            if iter_num % 250 == 0:
                ...
                with torch.no_grad():
                    ...
                    for loader, res_tag in loaders:
                        ...
                        #                         desc=f"Running Validation on Iter Num: {iter_num}", 
                        #                         total=num_batches,
                        #                         position=1,
                        #                         leave=True,
                        #                         disable=True)
